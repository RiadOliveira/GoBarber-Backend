...
let ResetPasswordService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!userToken) {
      throw new _AppError.default('Invalid token.', 400);
    }

    const tokenCreatedAt = userToken.createdAt;
    const comparedDate = (0, _dateFns.addHours)(tokenCreatedAt, 2);

    if ((0, _dateFns.isAfter)(Date.now(), comparedDate)) {
      throw new _AppError.default('Token expired.', 400);
    }

    const findedUser = await this.usersRepository.findById(userToken.userId);

    if (!findedUser) {
      throw new _AppError.default('Non-existing user', 400);
    }
