...
let ListProvidersService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(userId) {
    let users = await this.cacheProvider.recover(`providers-list:${userId}`);

    if (!users) {
      users = await this.usersRepository.findAllProviders({
        exceptUserId: userId
      });
      await this.cacheProvider.save(`providers-list:${userId}`, (0, _classTransformer.classToClass)(users));
    }
