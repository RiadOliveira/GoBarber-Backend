...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default("You can't create an appointment on a past date", 400);
    }

    if (userId === providerId) {
      throw new _AppError.default("You can't create an appointment with yourself", 400);
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default("You can't create an appointment out of the schedule", 400);
    }

    const findAppointment = await this.appointmentsRepository.findByDate(appointmentDate, providerId);

    if (findAppointment) {
      throw new _AppError.default('Date already marked', 400);
    }
    ...
    await this.cacheProvider.invalidate(`provider-appointments:${providerId}:${(0, _dateFns.format)(appointmentDate, 'yyyy-M-d')}`);
    return appointment;
  }
